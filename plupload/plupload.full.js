...
(function () {
    ...
    var g = {VERSION: "1.5b", STOPPED: 1, STARTED: 2, QUEUED: 1, UPLOADING: 2, FAILED: 4, DONE: 5, GENERIC_ERROR: -100, HTTP_ERROR: -200, IO_ERROR: -300, SECURITY_ERROR: -400, INIT_ERROR: -500, FILE_SIZE_ERROR: -600, FILE_EXTENSION_ERROR: -601, IMAGE_FORMAT_ERROR: -700, IMAGE_MEMORY_ERROR: -701, IMAGE_DIMENSIONS_ERROR: -702, mimeTypes: j, extend: function (o) {
    ...
    g.Uploader = function (r) {
        ...
        r = g.extend({chunk_size: 0, multipart: true, multi_selection: true, file_data_name: "upload", filters: []}, r);
        function s() {
            var w, x = 0, v;
    ...
    c.runtimes.Gears = c.addRuntime("gears", {getFeatures: function () {
        ...
        j.bind("PostInit", function () {
            ...
            c.addEvent(b.getElementById(n.browse_button), "click", function (s) {
                ...
                k.openFiles(g, {singleFile: !n.multi_selection, filter: r})
            }, j.id)
        });
        j.bind("UploadFile", function (s, p) {
            ...
            function n() {
                var z, B, w = s.settings.multipart, v = 0, A = {name: p.target_name || p.name}, x = s.settings.url;
                ...
                z.onreadystatechange = function () {
                    ...
                    if (z.readyState == 4) {
                        if (z.status == 200) {
                            C = {chunk: u, chunks: t, response: z.responseText, status: z.status};
                            ...
                            if (++u >= t) {
                                p.status = c.DONE;
                                s.trigger("FileUploaded", p, {response: z.responseText, status: z.status})
                            } else {
                                n()
                            }
                        } else {
                            s.trigger("Error", {code: c.HTTP_ERROR, message: c.translate("HTTP Error."), file: p, chunk: u, chunks: t, status: z.status})
                        }
                    }
                };
                if (u < t) {
        ...
        j.bind("Destroy", function (m) {
            var n, o, p = {browseButton: m.settings.browse_button, dropElm: m.settings.drop_element};
            for (n in p) {
...
(function (f, b, d, e) {
    ...
    d.runtimes.Flash = d.addRuntime("flash", {getFeatures: function () {
        return{jpgresize: true, pngresize: true, maxWidth: 8091, maxHeight: 8091, chunks: true, progress: true, multipart: true}
    }, init: function (k, p) {
        ...
        d.extend(j.style, {position: "absolute", top: "0px", background: k.settings.shim_bgcolor || "transparent", zIndex: 99999, width: "100%", height: "100%"});
        ...
        h.appendChild(j);
        l = "id=" + escape(k.id);
        j.innerHTML = '<object id="' + k.id + '_flash" width="100%" height="100%" style="outline:0" type="application/x-shockwave-flash" data="' + k.settings.flash_swf_url + '"><param name="movie" value="' + k.settings.flash_swf_url + '" /><param name="flashvars" value="' + l + '" /><param name="wmode" value="transparent" /><param name="allowscriptaccess" value="always" /></object>';
        function n() {
        ...
        k.bind("Flash:Init", function () {
            var s = {}, r;
            n().setFileFilters(k.settings.filters, k.settings.multi_selection);
            if (g[k.id]) {
                return
            }
            g[k.id] = true;
            k.bind("UploadFile", function (t, v) {
                var w = t.settings, u = k.settings.resize || {};
                n().uploadFile(s[v.id], w.url, {name: v.target_name || v.name, mime: d.mimeTypes[v.name.replace(/^.+\.([^.]+)/, "$1").toLowerCase()] || "application/octet-stream", chunk_size: w.chunk_size, width: u.width, height: u.height, quality: u.quality, multipart: w.multipart, multipart_params: w.multipart_params || {}, file_data_name: w.file_data_name, format: /\.(jpg|jpeg)$/i.test(v.name) ? "jpg" : "png", headers: w.headers, urlstream_upload: w.urlstream_upload})
            });
            k.bind("Flash:UploadProcess", function (u, t) {
            ...
            k.bind("Flash:UploadChunkComplete", function (t, v) {
            ...
            k.bind("Flash:SelectFiles", function (t, w) {
                var v, u, x = [], y;
                ...
                if (x.length) {
                    k.trigger("FilesAdded", x)
                }
            });
            k.bind("Flash:SecurityError", function (t, u) {
                k.trigger("Error", {code: d.SECURITY_ERROR, message: d.translate("Security error."), details: u.message, file: k.getFile(s[u.id])})
            });
            k.bind("Flash:GenericError", function (t, u) {
                k.trigger("Error", {code: d.GENERIC_ERROR, message: d.translate("Generic error."), details: u.message, file: k.getFile(s[u.id])})
            });
            k.bind("Flash:IOError", function (t, u) {
                k.trigger("Error", {code: d.IO_ERROR, message: d.translate("IO error."), details: u.message, file: k.getFile(s[u.id])})
            });
            k.bind("Flash:ImageError", function (t, u) {
                k.trigger("Error", {code: parseInt(u.code, 10), message: d.translate("Image error."), file: k.getFile(s[u.id])})
            });
            k.bind("Flash:StageEvent:rollOver", function (t) {
                var u, v;
                u = b.getElementById(k.settings.browse_button);
                v = t.settings.browse_button_hover;
                if (u && v) {
                    d.addClass(u, v)
                }
            });
            k.bind("Flash:StageEvent:rollOut", function (t) {
                var u, v;
                u = b.getElementById(k.settings.browse_button);
                v = t.settings.browse_button_hover;
                if (u && v) {
            ...
            k.bind("Flash:StageEvent:mouseDown", function (t) {
                var u, v;
                u = b.getElementById(k.settings.browse_button);
                v = t.settings.browse_button_active;
                if (u && v) {
            ...
            k.bind("Flash:StageEvent:mouseUp", function (t) {
                var u, v;
                u = b.getElementById(k.settings.browse_button);
                v = t.settings.browse_button_active;
                if (u && v) {
            ...
            k.bind("Flash:ExifData", function (t, u) {
                k.trigger("ExifData", k.getFile(s[u.id]), u.data)
            });
            k.bind("Flash:GpsData", function (t, u) {
                k.trigger("GpsData", k.getFile(s[u.id]), u.data)
            });
            k.bind("QueueChanged", function (t) {
                k.refresh()
            });
            k.bind("FilesRemoved", function (t, v) {
                var u;
                for (u = 0; u < v.length; u++) {
                    n().removeFile(s[v[u].id])
                }
            });
            k.bind("StateChanged", function (t) {
                k.refresh()
            });
            k.bind("Refresh", function (t) {
                var u, v, w;
                n().setFileFilters(k.settings.filters, k.settings.multi_selection);
                u = b.getElementById(t.settings.browse_button);
                if (u) {
            ...
            k.bind("Destroy", function (t) {
                var u;
                d.removeAllEvents(b.body, t.id);
                delete g[t.id];
                delete a[t.id];
                u = b.getElementById(t.id + "_flash_container");
                if (u) {
                    h.removeChild(u)
                }
            });
        ...
        function b() {
            g.bind("PostInit", function () {
                ...
                a.addEvent(document.getElementById(d.browse_button), "click", function (w) {
                    ...
                    e.FileBrowse.OpenBrowseDialog({mimeTypes: u}, function (x) {
                        if (x.success) {
                            f(x.value)
                        }
                    })
...
(function (h, k, j, e) {
    ...
    j.runtimes.Html5 = j.addRuntime("html5", {getFeatures: function () {
        ...
        p = (function () {
            ...
            return{ie: !v && !u && (/MSIE/gi).test(x) && (/Explorer/gi).test(y.appName), webkit: v, gecko: !v && /Gecko/.test(x), safari: w, safariwin: w && navigator.platform.indexOf("Win") !== -1, opera: !!u}
        }());
        o = s = q = n = false;
        if (h.XMLHttpRequest) {
        ...
        if (o) {
            ...
            q = !!(File && (File.prototype.getAsDataURL || h.FileReader) && r);
        ...
        return{html5: o, dragdrop: (function () {
            ...
            return("draggable" in u) || ("ondragstart" in u && "ondrop" in u)
        }()), jpgresize: q, pngresize: q, multipart: q || !!h.FileReader || !!h.FormData, canSendBinary: r, cantSendBlobInFormData: !!(p.gecko && h.FormData && h.FileReader && !FileReader.prototype.readAsArrayBuffer), progress: s, chunks: n, triggerDialog: (p.gecko && h.FormData || p.webkit)}
    }, init: function (p, q) {
        var n;
        ...
        p.bind("Init", function (v) {
            ...
            F.innerHTML = '<input id="' + p.id + '_html5"  style="font-size:999px" type="file" accept="' + B.join(",") + '" ' + (p.settings.multi_selection ? 'multiple="multiple"' : "") + " />";
            F.scrollTop = 100;
        ...
        p.bind("UploadFile", function (r, t) {
            ...
            function v(z) {
                var C = 0, B = 0, y = ("FileReader" in h) ? new FileReader : null, D = typeof(z) === "string";

                function A() {
                    var I, M, K, L, H, J, F, E = r.settings.url;

                    function G(W) {
                        var T = 0, U = new XMLHttpRequest, X = U.upload, N = "----pluploadboundary" + j.guid(), O, P = "--", V = "\r\n", R = "";
                        if (X) {
                            X.onprogress = function (Y) {
                                t.loaded = Math.min(t.size, B + Y.loaded - T);
                        ...
                        U.onreadystatechange = function () {
                           ...
                           // alert("onreadystratechange");
                            ...
                            if (U.readyState == 4) {
                                ...
                                if (Y >= 400) {
                                    r.trigger("Error", {code: j.HTTP_ERROR, message: j.translate("HTTP Error."), file: t, status: Y})
                                } else {
                                    if (K) {
                                        aa = {chunk: C, chunks: K, response: U.responseText, status: Y};
                                        r.trigger("ChunkUploaded", t, aa);
                                        B += J;
                                        if (aa.cancelled) {
                                            t.status = j.FAILED;
                                            return
                                        }
                                        t.loaded = Math.min(t.size, (C + 1) * H)
                                    } else {
                                        t.loaded = t.size
                                    }
                                    r.trigger("UploadProgress", t);
                        ...
                        if (r.settings.multipart && n.multipart) {
                            ...
                            U.open("post", E, true);
                            j.each(r.settings.headers, function (Z, Y) {
                                U.setRequestHeader(Y, Z)
                            });
                            if (!D && !!h.FormData) {
                                O = new FormData();
                                j.each(j.extend(L, r.settings.multipart_params), function (Z, Y) {
                                    O.append(Y, Z)
                                });
                                O.append(r.settings.file_data_name, W);
                                U.send(O);
                                return
                            }
                            if (D) {
                                U.setRequestHeader("Content-Type", "multipart/form-data; boundary=" + N);
                                j.each(j.extend(L, r.settings.multipart_params), function (Z, Y) {
                                    R += P + N + V + 'Content-Disposition: form-data; name="' + Y + '"' + V + V;
                                    R += unescape(encodeURIComponent(Z)) + V
                                });
                                F = j.mimeTypes[t.name.replace(/^.+\.([^.]+)/, "$1").toLowerCase()] || "application/octet-stream";
                                R += P + N + V + 'Content-Disposition: form-data; name="' + r.settings.file_data_name + '"; filename="' + unescape(encodeURIComponent(t.name)) + '"' + V + "Content-Type: " + F + V + V + W + V + P + N + P + V;
                                T = R.length - W.length;
                                W = R;
                                if (U.sendAsBinary) {
                                    U.sendAsBinary(W)
                                    ...
                                    if (n.canSendBinary) {
                                        var S = new Uint8Array(W.length);
                                        for (var Q = 0; Q < W.length; Q++) {
                                            S[Q] = (W.charCodeAt(Q) & 255)
                                        }
                                        U.send(S.buffer)
                                    }
                                }
                        ...
                        E = j.buildUrl(r.settings.url, j.extend(L, r.settings.multipart_params));
                        U.open("post", E, true);
                    ...
                    if (u.chunk_size && t.size > u.chunk_size && (n.chunks || typeof(z) == "string")) {
                        H = u.chunk_size;
                        K = Math.ceil(t.size / H);
                        J = Math.min(H, t.size - (C * H));
                        if (typeof(z) == "string") {
                            I = z.substring(C * H, C * H + J)
                        } else {
                            I = w(z, C * H, C * H + J)
                        }
                        L.chunk = C;
                    ...
                    if (y && n.cantSendBlobInFormData && n.chunks && r.settings.chunk_size) {
                        y.onload = function () {
                            D = true;
                            G(y.result)
                        };
...
(function (d, a, b, c) {
    ...
    b.runtimes.Html4 = b.addRuntime("html4", {getFeatures: function () {
        var f = (function () {
            ...
            return{ie: !h && !g && (/MSIE/gi).test(k) && (/Explorer/gi).test(l.appName), webkit: h, gecko: !h && /Gecko/.test(k), safari: j, safariwin: j && navigator.platform.indexOf("Win") !== -1, opera: !!g}
        }());
        return{multipart: true, triggerDialog: (f.gecko && d.FormData || f.webkit)}
    }, init: function (f, g) {
        f.bind("Init", function (p) {
            ...
            function v() {
                ...
                b.extend(A.style, {width: "100%", height: "100%", opacity: 0, fontSize: "999px"});
